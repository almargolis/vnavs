...
DISPLAY_MODE_DOWNLOAD = (
    "Download"  # in process of downloading mission, play as it comes
)
...
class MissionControl(vmqtt.mqtt_node):
    ...
    def OnMissionLoad(self):
        ...
        f.close()
        self.mission = navigator.Mission(
            name=mission_name, script=mission_script.split("\n")
        )
...
def RunGps(waypoint):
    ...
    if waypoint is not None:
        ...
        value_payload = vmqtt.Publish(
            vconst.data_get_topic, payload, ResponseTopic="data/value"
        )
    ...
    while True:
        ...
        if have_new_position_data:
            ...
            print(
                "Distance:",
                d.distance_to_waypoint,
                "Heading:",
                d.heading_to_waypoint,
                "Speed:",
                gps_device.data.gps_speed,
                "Quality:",
                gps_device.data.gps_quality,
            )
    ...
    while True:
        ...
        if have_new_position_data:
            ...
            if previous_position is not None:
                ...
                print(
                    "{:f} {:f}".format(
                        distance_from_last_position, accumulated_movement
                    )
                )
            ...
            print(
                "CENTER:",
                center_position.DMS(),
                distance_from_previous_center,
                survey_map.survey_hypotenuse,
                "LOCATION:",
...
def DistanceGps():
    ...
    print(
        "Current GPS {}, distance to goal {:f}".format(
            new_position.DMS(), dw.distance_to_waypoint
        )
    )
    if start_name is not None:
        ...
        print(
            "Relative GPS {}, distance to goal {:f}, distance to relative start {:f}".format(
                start_position.DMS(), dw.distance_to_waypoint, dw2.distance_to_waypoint
            )
        )
