...
class Test(object):
    ...
    def __init__(self, fld_ct, rec_ct):
        ...
        for ix in range(self.fld_ct):
            ...
            insert_parms.append("?")
        self.insert_query = "INSERT INTO MissionLog VALUES (%s);" % ", ".join(
            insert_parms
        )

        self.cursor.execute(
            "CREATE TABLE MissionLog ({})".format(", ".join(sql_field_list))
        )
    ...
    def RunTest(self):
        ...
        print(
            ...
            % (self.rec_ct, elapsed_time, self.rec_ct / elapsed_time)
        )
