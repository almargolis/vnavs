...
GpsDataDict.AddAttribute(
    GPS_LONGITUDE, DDT_FLOAT, default_value=0, min_value=0.0, limit_value=360.0
)
GpsDataDict.AddAttribute(
    GPS_LATITUDE, DDT_FLOAT, default_value=0, min_value=0.0, limit_value=360.0
)
GpsDataDict.AddAttribute(
    RAW_GPS_LONGITUDE, DDT_FLOAT, default_value=0, min_value=0.0, limit_value=360.0
)
GpsDataDict.AddAttribute(
    RAW_GPS_LATITUDE, DDT_FLOAT, default_value=0, min_value=0.0, limit_value=360.0
)
GpsDataDict.AddAttribute(
    GPS_QUALITY,
    DDT_STR,
...
GpsDataDict.AddAttribute(
    GPS_DIFFERENTIAL, DDT_STR, values={"A": "Autonomous", "D": "Differential GPS"}
)
...
ImuDataDict.AddAttribute(
    IMU_YAW, DDT_FLOAT, uom="degrees", default_value=0, min_value=0.0, limit_value=360.0
)
...
class Position(object):
    ...
    def DistanceToWaypoint(self, waypoint):
        ...
        (
            position_latitude_radians,
            position_longitude_radians,
            waypoint_latitude_radians,
            waypoint_longitude_radians,
        ) = map(
            math.radians,
            [self.latitude, self.longitude, waypoint.latitude, waypoint.longitude],
        )
        longitude_difference_radians = (
            waypoint_longitude_radians - position_longitude_radians
        )
        latitude_difference_radians = (
            waypoint_latitude_radians - position_latitude_radians
        )
        a = math.sin(latitude_difference_radians / 2) ** 2 + (
            math.cos(position_latitude_radians)
            * math.cos(waypoint_latitude_radians)
        ...
        y = math.cos(position_latitude_radians) * math.sin(
            waypoint_latitude_radians
        ) - (
            math.sin(position_latitude_radians)
            * math.cos(waypoint_latitude_radians)
        ...
        o = HelmOrder(
            heading_to_waypoint, distance_to_waypoint  # compass degrees
        )  # meters - haversine / great circle distance
        return o
...
class GpsDevice(object):
    ...
    def Read(self):
        ...
        if self.next_eol_ix >= 0:
            ...
            self.gps_buffer += self.gps_port.read(size=1024).decode(
    ...
    def StartRelativeGpsPositioning(self, position):
        assert isinstance(position, Position), (
            "invalid position class " + position.__class__.__name__
        )
    ...
    def UpdateRmcSentence(self, parsed_sentence):
        ...
        if self.relative_known_start_position is None:
            ...
            print(
                "UpdateRmcSentence()",
                self.data.gps_latitude.__class__.__name__,
                self.data.gps_latitude,
                self.data.gps_longitude.__class__.__name__,
        ...
        if heading_raw == "":
            ...
            try:
                self.data.gps_heading = float(
                    heading_raw
                )  # degrees clockwise from North
            except ValueError:
                print("Invalid RMC heading", repr(heading_raw))
        self.gps_differential = parsed_sentence.data[
            11
        ]  # A=autonomous, D=differeential GPS
        """
...
def TestImu():
    ...
    while True:
        ...
        print(
            "IMU P: {:+09.4f}  R: {:+09.4f}  Y: {:+09.4f} {:+09.4f}".format(
                o["pitch"], o["roll"], o["yaw"], p["yaw"] - o["yaw"]
            )
        )
        p = o
        time.sleep(0.1)
